...
const baseSchema = {
  tool: z.enum(["text-to-image", "text-to-video"]),
...
// Called once per take, right after a Text to Image/Video render settles
...
export const POST = withDepCutAuth(async (request) => {
  // A rendered video (sometimes an image, past MIN_OFFLOAD_BYTES) travels as
  // a stored-blob placeholder rather than inline base64 — see hostedBlobs.ts.
  // Resolve it back to dataBase64 before validating; a small payload that
  // never got offloaded passes through unchanged.
