...
APP_DIR="$ROOT_DIR/dist/DepCut.app"
DMG_PATH="$ROOT_DIR/dist/DepCut.dmg"
DMG_RW_PATH="$ROOT_DIR/dist/DepCut-rw.dmg"
DMG_ROOT="$ROOT_DIR/dist/DepCutInstaller"
RUNTIME_PACKAGE_DIR="$ROOT_DIR/dist/LocalRuntimePackages"
APP_VERSION="${DEPCUT_APP_VERSION:-0.1.0}"
APP_BUILD="${DEPCUT_APP_BUILD:-1}"
SPARKLE_FEED_URL="${DEPCUT_SPARKLE_FEED_URL:-}"
SPARKLE_PUBLIC_ED_KEY="${DEPCUT_SPARKLE_PUBLIC_ED_KEY:-}"
CONTENTS_DIR="$APP_DIR/Contents"
MACOS_DIR="$CONTENTS_DIR/MacOS"
RESOURCES_DIR="$CONTENTS_DIR/Resources"
FRAMEWORKS_DIR="$CONTENTS_DIR/Frameworks"
BUILD_DIR="$ROOT_DIR/apps/DepCut"
EXECUTABLE="$BUILD_DIR/.build/release/DepCut"
CACHE_DIR="$BUILD_DIR/.build/package-cache"
APP_ICON_SOURCE="$BUILD_DIR/Sources/DepCut/Resources/DepCut.icns"
APP_ICONSET_SOURCE="$BUILD_DIR/Sources/DepCut/Resources/DepCut.iconset"
DMG_BACKGROUND_SOURCE="$ROOT_DIR/scripts/assets/depcut-dmg-background.svg"
DMG_BACKGROUND_RENDERED="$ROOT_DIR/dist/depcut-dmg-background.png"
...
set_dmg_volume_icon() {
  local volume_dir="$1"
  local volume_icon_source="$RESOURCES_DIR/DepCut.icns"
...
tell application "Finder"
  ...
  set backgroundFile to POSIX file "$mount_dir/.background/depcut-dmg-background.png" as alias
  open mountedFolder
  delay 0.2
  ...
  set position of item "DepCut.app" of installerWindow to {$DMG_APP_ICON_X, $DMG_APP_ICON_Y}
...
create_drag_to_applications_dmg() {
  ...
  mount_dir="$(mktemp -d "${TMPDIR:-/tmp}/depcut-dmg.XXXXXX")"
  detach_dmg_mount() {
  ...
  cp -R "$APP_DIR" "$DMG_ROOT/DepCut.app"
  ln -s /Applications "$DMG_ROOT/Applications"
  cp "$DMG_BACKGROUND_RENDERED" "$DMG_ROOT/.background/depcut-dmg-background.png"
...
# DEPCUT_APP_SIGN_IDENTITY selects how the app is signed:
...
APP_SIGN_IDENTITY="${DEPCUT_APP_SIGN_IDENTITY:--}"
APP_ENTITLEMENTS="${DEPCUT_APP_ENTITLEMENTS:-$ROOT_DIR/scripts/assets/depcut.entitlements}"
...
sign_app() {
  ...
  [ -n "${DEPCUT_SIGN_KEYCHAIN:-}" ] && base=(--keychain "$DEPCUT_SIGN_KEYCHAIN" "${base[@]}")
...
notarize_app() {
  ...
  local zip="$ROOT_DIR/dist/DepCut-app-notarize.zip"
  rm -f "$zip"
...
notarize_dmg() {
  ...
  [ -n "${DEPCUT_SIGN_KEYCHAIN:-}" ] && dmg_sign=(--keychain "$DEPCUT_SIGN_KEYCHAIN" "${dmg_sign[@]}")
...
if [ -n "${DEPCUT_SIGN_KEYCHAIN:-}" ]; then
  mkdir -p "$HOME/Library/Preferences"
  security list-keychains -d user -s "$DEPCUT_SIGN_KEYCHAIN" $(security list-keychains -d user | sed -e 's/"//g')
  security default-keychain -d user -s "$DEPCUT_SIGN_KEYCHAIN"
fi

cd "$BUILD_DIR"
echo "Compiling DepCut for Mac ..."
swift build -c release --product DepCut
...
# in bundled-tools.json, or building from source). Point DEPCUT_TOOLS_DIR at a prebuilt set to
# use that instead.
...
stage_bundled_tools() {
  # ensure-bundled-tools.sh either populates vendor/depcut-tools or, with DEPCUT_TOOLS_DIR
  ...
  local source_dir="${DEPCUT_TOOLS_DIR:-$ROOT_DIR/vendor/depcut-tools}"
  local dest_dir="$RESOURCES_DIR/depcut-tools"
  rm -rf "$dest_dir"
  mkdir -p "$dest_dir"
  ...
  DEPCUT_TOOLS_SIGN_IDENTITY="$APP_SIGN_IDENTITY" \
...
stage_cut_engine() {
  ...
  local source_bin="${DEPCUT_CUT_ENGINE_BIN:-}"
  if [ -z "$source_bin" ]; then
    echo "Building the DepCut engine..."
    bash "$ROOT_DIR/site/scripts/build-cut-engine.sh"
    source_bin="$ROOT_DIR/site/dist/cut-engine/depcut-cut-engine-$arch"
  fi
  if [ ! -f "$source_bin" ]; then
    echo "DepCut engine binary not found at $source_bin." >&2
    exit 1
  fi
  local dest_dir="$RESOURCES_DIR/cut-engine"
  mkdir -p "$dest_dir"
  cp "$source_bin" "$dest_dir/depcut-cut-engine"
  chmod +x "$dest_dir/depcut-cut-engine"
...
PLIST
  ...
  [ -n "${DEPCUT_SIGN_KEYCHAIN:-}" ] && sopts+=(--keychain "$DEPCUT_SIGN_KEYCHAIN")
  codesign "${sopts[@]}" "$dest_dir/depcut-cut-engine" >/dev/null 2>&1 || true
  rm -f "$ents"
  echo "Bundled the DepCut engine from $source_bin."
  ...
  # no JIT entitlements.
  local source_stt="${DEPCUT_CUT_STT_BIN:-$(dirname "$source_bin")/cut-stt-$arch}"
  if [ ! -f "$source_stt" ]; then
    echo "DepCut speech tool not found at $source_stt." >&2
    exit 1
  fi
  ...
  [ -n "${DEPCUT_SIGN_KEYCHAIN:-}" ] && topts+=(--keychain "$DEPCUT_SIGN_KEYCHAIN")
  codesign "${topts[@]}" "$dest_dir/cut-stt" >/dev/null 2>&1 || true
  echo "Bundled the DepCut speech tool from $source_stt."
}

rm -rf "$APP_DIR"
mkdir -p "$MACOS_DIR" "$RESOURCES_DIR" "$FRAMEWORKS_DIR"
cp "$EXECUTABLE" "$MACOS_DIR/DepCut"
if ! otool -l "$MACOS_DIR/DepCut" | grep -q "@executable_path/../Frameworks"; then
  if ! command -v install_name_tool >/dev/null 2>&1; then
    echo "install_name_tool is required to package DepCut.app with embedded frameworks." >&2
    exit 1
  fi
  install_name_tool -add_rpath "@executable_path/../Frameworks" "$MACOS_DIR/DepCut"
fi

rm -rf "$RUNTIME_PACKAGE_DIR"

# Copy every SwiftPM-generated resource bundle, not just the executable target's.
# Each target with resources (DepCut, DepCutRuntime, ...) produces its own
# DepCut_<Target>.bundle, and the generated Bundle.module accessor fatalErrors at
# launch if its bundle is missing. Missing DepCut_DepCutRuntime.bundle is what made
...
if [ -n "$RELEASE_DIR" ]; then
  for resource_bundle in "$RELEASE_DIR"/DepCut_*.bundle; do
    [ -d "$resource_bundle" ] || continue
    cp -R "$resource_bundle" "$RESOURCES_DIR/"
    copied_any_bundle=1
  done
fi
if [ "$copied_any_bundle" -eq 0 ] && [ -d "$BUILD_DIR/.build/release/DepCut_DepCut.resources" ]; then
  cp -R "$BUILD_DIR/.build/release/DepCut_DepCut.resources/." "$RESOURCES_DIR/"
  copied_any_bundle=1
fi
...
<dict>
  ...
  <string>DepCut</string>
  <key>CFBundleIdentifier</key>
  <string>com.depcutuse.DepCut</string>
  ...
  <string>DepCut records microphone audio when you record your screen.</string>
  <key>NSSpeechRecognitionUsageDescription</key>
  <string>DepCut transcribes your video's audio on-device to build subtitles.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>DepCut records your screen when you start a screen recording.</string>
  ...
  <string>DepCut saves your screen recordings to the Desktop.</string>
$SPARKLE_PLIST_KEYS
</dict>
</plist>
PLIST
...
echo "For Sparkle updates, package with DEPCUT_SPARKLE_FEED_URL and DEPCUT_SPARKLE_PUBLIC_ED_KEY."
