...
/** The standalone Text to Image page's persisted output — same split as
 * VISUAL_GENERATION_PREFIX above. */
export const IMAGE_GENERATION_PREFIX = "image-generations/";
export const imageGenerationKey = (userId: string, id: string, fileName: string) =>
  `${IMAGE_GENERATION_PREFIX}${userId}/${id}/${fileName}`;
