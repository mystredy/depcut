...
APP_DIR="$ROOT_DIR/apps/DepCut"
LOG_SCRIPT="$ROOT_DIR/scripts/tail-depcut-logs.sh"
BUILD_PRODUCTS_DIR="$APP_DIR/.build/debug"
DEPCUT_BIN="$BUILD_PRODUCTS_DIR/DepCut"
DEV_BUNDLE_IDENTIFIER="${DEPCUT_DEV_BUNDLE_IDENTIFIER:-com.depcutuse.DepCut.dev}"
DEV_DISPLAY_NAME="${DEPCUT_DEV_DISPLAY_NAME:-DepCut Dev}"
DEV_EXECUTABLE_NAME="${DEPCUT_DEV_EXECUTABLE_NAME:-$DEV_DISPLAY_NAME}"
...
# The dev bundle wears the red icon (DepCut.icns hue-shifted, regenerate with
# `magick -modulate 100,100,178` per iconset PNG + iconutil) so it never gets
# mistaken for an installed release in the Dock.
APP_ICON_SOURCE="$APP_DIR/Sources/DepCut/Resources/DepCut-dev.icns"
APP_ICONSET_SOURCE="$APP_DIR/Sources/DepCut/Resources/DepCut.iconset"
# The dev engine binds its own loopback port (baked into the dev Info.plist below,
# read by DepCutEngineSupervisor) so it coexists with an installed release's
# engine on 41417 instead of the two supervisors killing each other's engine.
DEV_CUT_ENGINE_PORT="${DEPCUT_DEV_CUT_ENGINE_PORT:-41418}"
LAUNCH_APP="${DEPCUT_LAUNCH_APP:-1}"
SITE_PID=""
SITE_LOG="${DEPCUT_SITE_LOG:-${TMPDIR:-/tmp}/depcut-site-dev.log}"
LOG_PID=""

export DEPCUT_WEB_BASE_URL="${DEPCUT_WEB_BASE_URL:-http://localhost:3000}"

# Stop only a prior DEV build so a rebuild (or script exit) doesn't leave two dev
# instances running. Never touch an installed release: its process is "DepCut" from
# "DepCut.app", while the dev build runs as "$DEV_EXECUTABLE_NAME" from
# "$DEV_DISPLAY_NAME.app" with a separate bundle id and Cut engine port,
# so the two are meant to run side by side.
stop_running_dev_depcut_apps() {
...
cleanup_child_processes() {
  if [ "$LAUNCH_APP" != "0" ] && [ "${DEPCUT_KEEP_APP_ON_EXIT:-0}" != "1" ]; then
    stop_running_dev_depcut_apps
  fi
...
start_logger() {
  if [ "${DEPCUT_TAIL_LOGS:-1}" = "0" ]; then
    echo "Skipping DepCut log tail because DEPCUT_TAIL_LOGS=0."
    return
  fi

  if [ ! -x "$LOG_SCRIPT" ]; then
    echo "Skipping DepCut log tail because $LOG_SCRIPT is not executable." >&2
    return
  fi

  local log_args=()
  log_args+=(--process "$DEV_EXECUTABLE_NAME")
  if [ "${DEPCUT_LOG_ERRORS_ONLY:-0}" = "1" ]; then
    log_args+=(--errors)
  fi
  if [ "${DEPCUT_LOG_DEBUG:-0}" = "1" ]; then
    log_args+=(--debug)
  fi
  if [ -n "${DEPCUT_LOG_CONTAINS:-}" ]; then
    log_args+=(--contains "$DEPCUT_LOG_CONTAINS")
  fi
  if [ "${DEPCUT_LOG_ALL:-0}" != "1" ]; then
    log_args+=(--subsystem "${DEPCUT_LOG_SUBSYSTEM:-com.depcut.app}")
  elif [ -n "${DEPCUT_LOG_SUBSYSTEM:-}" ]; then
    log_args+=(--subsystem "$DEPCUT_LOG_SUBSYSTEM")
  fi

  echo "Tailing DepCut logs..."
...
site_health_url() {
  printf '%s/api/health' "${DEPCUT_WEB_BASE_URL%/}"
}
...
ensure_site_server() {
  if [ "${DEPCUT_START_SITE:-1}" = "0" ]; then
    echo "Skipping site dev server because DEPCUT_START_SITE=0."
    return
  fi

  if ! is_local_web_base_url; then
    echo "Skipping site dev server because DEPCUT_WEB_BASE_URL is not local: $DEPCUT_WEB_BASE_URL"
    return
  fi

  if site_is_running; then
    echo "Site dev server is already running at $DEPCUT_WEB_BASE_URL."
    return
  fi
  ...
  echo "Starting site dev server at $DEPCUT_WEB_BASE_URL ..."
  (
    cd "$ROOT_DIR/site"
    npm run dev
  ) >"$SITE_LOG" 2>&1 &
  ...
  local wait_seconds="${DEPCUT_SITE_WAIT_SECONDS:-30}"
  local elapsed=0
...
resolve_codesign_identity() {
  if [ -n "${DEPCUT_CODESIGN_IDENTITY:-}" ]; then
    printf '%s\n' "$DEPCUT_CODESIGN_IDENTITY"
    return
  fi
...
ensure_apple_wwdr_intermediate() {
  ...
  # dev needs no chain.
  [ "${DEPCUT_CODESIGN_IDENTITY:-}" = "-" ] && return
...
prepare_app_icon() {
  local destination="$RESOURCES_DIR/DepCut.icns"
  ...
  if [ -d "$APP_ICONSET_SOURCE" ]; then
    if ! command -v iconutil >/dev/null 2>&1; then
      echo "iconutil is required to package DepCut.app from $APP_ICONSET_SOURCE." >&2
      exit 1
    fi
...
<dict>
  ...
  <true/>
  <key>DepCutEnginePort</key>
  <integer>$DEV_CUT_ENGINE_PORT</integer>
  <key>NSMicrophoneUsageDescription</key>
  <string>DepCut uses the microphone for user-requested voice input.</string>
  <key>NSSpeechRecognitionUsageDescription</key>
  <string>DepCut transcribes your voice input on-device to turn it into a command.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>DepCut captures bounded screenshots for user-requested app context.</string>
  <key>NSAppleEventsUsageDescription</key>
  <string>DepCut uses local app automation only for user-requested actions.</string>
  <key>NSDesktopFolderUsageDescription</key>
  <string>DepCut may search Desktop files only when you ask it to find or open a local item.</string>
  <key>NSDocumentsFolderUsageDescription</key>
  <string>DepCut may search Documents files only when you ask it to find or open a local item.</string>
  <key>NSDownloadsFolderUsageDescription</key>
  <string>DepCut may search Downloads files only when you ask it to find or open a local item.</string>
  <key>NSAppleMusicUsageDescription</key>
  <string>DepCut plays Apple Music natively when you ask for music.</string>
</dict>
</plist>
PLIST
}
...
create_debug_app_bundle() {
  ...
  rm -rf "$DEV_APP_DIR" "$BUILD_PRODUCTS_DIR/DepCut.app"
  mkdir -p "$MACOS_DIR" "$RESOURCES_DIR" "$FRAMEWORKS_DIR"

  cp "$DEPCUT_BIN" "$MACOS_DIR/$DEV_EXECUTABLE_NAME"
  if ! otool -l "$MACOS_DIR/$DEV_EXECUTABLE_NAME" | grep -q "@executable_path/../Frameworks"; then
    if ! command -v install_name_tool >/dev/null 2>&1; then
      echo "install_name_tool is required to package the DepCut debug app." >&2
      exit 1
    fi
  ...
  while IFS= read -r bundle; do
    ...
    copied_any_bundle=1
  done < <(find "$APP_DIR/.build" -name "DepCut_*.bundle" -type d -path "*/debug/*" | grep -v "Tests")

  if [ "$copied_any_bundle" -eq 0 ]; then
    echo "Missing SwiftPM resource bundles for DepCut." >&2
    exit 1
  fi

  # Stage the bundled CLI tools at Contents/Resources/depcut-tools so bundledToolsDirectory
  ...
  if [ -d "$ROOT_DIR/vendor/depcut-tools" ]; then
    ln -sfn "$ROOT_DIR/vendor/depcut-tools" "$RESOURCES_DIR/depcut-tools"
  fi

  # Build and stage the DepCut engine so the app supervises a current copy — no separate
  # manual build step. Non-fatal: a Cut build hiccup shouldn't block DepCut dev (the app just
  # runs without Cut).
  cut_engine_arch="$(uname -m)"
  [ "$cut_engine_arch" = "x86_64" ] && cut_engine_arch="x64"
  cut_engine="$ROOT_DIR/site/dist/cut-engine/depcut-cut-engine-$cut_engine_arch"
  if [ -d "$ROOT_DIR/site" ]; then
    echo "Building DepCut engine..."
    bash "$ROOT_DIR/site/scripts/build-cut-engine.sh" ||
      echo "Warning: Cut engine build failed; running DepCut without Cut." >&2
  fi
  if [ -f "$cut_engine" ]; then
    mkdir -p "$RESOURCES_DIR/cut-engine"
    ln -sf "$cut_engine" "$RESOURCES_DIR/cut-engine/depcut-cut-engine"
  fi
...
if [ "${DEPCUT_STOP_APPS_BEFORE_BUILD:-1}" = "1" ]; then
  echo "Stopping any running DepCut Dev app..."
  stop_running_dev_depcut_apps
fi

cd "$APP_DIR"

echo "Building DepCut..."
swift_build

if [ ! -x "$DEPCUT_BIN" ]; then
  echo "Built DepCut executable was not found at $DEPCUT_BIN." >&2
  exit 1
fi
...
if [ "$LAUNCH_APP" = "0" ]; then
  echo "Skipping DepCut launch because DEPCUT_LAUNCH_APP=0."
  exit 0
fi

echo "Starting DepCut..."
start_logger
