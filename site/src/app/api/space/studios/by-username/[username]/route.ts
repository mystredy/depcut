...
// Same shape as GET /api/space/studios/[id] — the entry point for the
...
export const GET = withDepCutAuth(async (request: DepCutAuthenticatedRequest, context: RouteContext) => {
  ...
  const space = await prisma.studio.findUnique({ where: { username } });
  if (!space) return notFoundResponse();

  const membership = await getStudioMembership(request.depcut.userId, space.id);
