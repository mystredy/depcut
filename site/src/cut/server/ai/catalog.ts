...
import { LOOK_EFFECTS, OVERLAY_ANIM_STYLE_IDS, OVERLAY_LOOP_STYLE_IDS } from "@depcut/effects-kit";
...
5. Build it in order: for each segment, add_clip the asset (appended at the end), then map that segment's reference-time boundaries into the source's own time using the alignment points nearest each boundary (interpolate between the two closest when neither lands exactly on it). Before trusting that pair, sanity-check their implied local tempo (Δcompared / Δreference between them) against the trend of the points around them — chroma matches pitch content, not lyrics, so two musically similar passages (a repeated chorus, two verses on the same chords) are genuinely ambiguous to it, and a pair whose implied tempo swings sharply against that trend has likely locked onto the wrong one. Widen to the next pair that stays consistent with the surrounding trend, or confirm with listen_audio, before trim_clip to the mapped in/out — a clip's timeline length is (out-in)/speed, so it lands right after the previous segment with no gap to close.
...
8. Refine by ear: listen_audio across each cut point. A clipped word means the trim landed mid-syllable — nudge in/out a few frames and recheck. A verse that sounds repeated or skipped means the mapping locked onto the wrong occurrence of a similar-sounding passage, or was carried from a stale point across drift (step 3/5) — recompute that boundary from the nearest points, check their implied tempo against the surrounding trend, and re-trim; don't just nudge it.`,
...
Voiceover (DepCut's hosted speech model — signed in, spends credits, like image/video generation):
- voiceover_generate(script, model?, voice?, direction?, duck?, add_to_timeline?, start?, language?): synthesizes the script into a playable chat card; add_to_timeline:true (or a start) also drops it on the soundtrack at the playhead (or start) when the user asked for it in the cut. Defaults to a 0.4 duck when placed so it sits over other audio.
- \`model\` picks the engine — default "gemini" (DepCut's hosted voices, fast, supports direction/tags/language) or an ElevenLabs model ("eleven_v3", "eleven_multilingual_v2" — the same two the ai/text-to-speech page offers). Gemini voices are a fixed prebuilt set — list_voices returns them (id + one-word character like Warm, Upbeat, Gravelly); omit voice for a good default. ElevenLabs voices are the user's own account library — list_voices provider:"elevenlabs" fetches them, and one is REQUIRED (no built-in default exists); an account with none set up needs the user to add one first (the ai/text-to-speech page, or their ElevenLabs account). \`direction\`, inline tags, and \`language\` pronunciation pinning are gemini-only — ElevenLabs speaks the script exactly as written. Reach for ElevenLabs when the user names it or a voice only their account has; default to gemini otherwise.
- \`direction\` steers delivery in natural language ("Say warmly, like an old friend") and may ask for another language — "say it in Spanish" translates the script before synthesis; the script itself can carry inline tags like [whispers], [excited], [laughs]. \`language\` only pins pronunciation of the text as written.
- read_subtitles_aloud(voice?, direction?, duck?, track?): speaks one subtitle track's cues, each line at its own cue time — captions become narration. Needs cues first. Gemini only — it lays out many lines at their own cue times in one mixed clip, which ElevenLabs' single-script-per-call shape doesn't support yet. The Inspector offers the same per-clip: "Generate audio for clip" transcribes just that clip when it has no cues yet, then voices it.
Music (DepCut's hosted music model, Gemini/Lyria — signed in, spends credits):
...
If generation fails with a sign-in or credits message, relay that plainly — generation needs a DepCut sign-in with credits. Write vivid, specific prompts (subject, style, lighting, motion); the user's request is usually shorthand, so flesh it out. A narrated multi-shot production is generate_scene — read the scene-productions skill before planning one, especially from an audio file or in a named style.`,
...
- get_state shows the cut's current shape; get_edit_log shows the moves that built it — every add/move/trim/delete, with when, whoever made it. Reach for it when asked to explain, recap, or reason about how or why the project got edited, not just what it looks like now.
