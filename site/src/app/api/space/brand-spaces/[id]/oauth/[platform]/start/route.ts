...
// Managers only. Opened in a popup window from a Brand Space's Repurpose
...
// scoped to this Brand Space via the signed state's ownerType, read by the
...
// platform, so a Brand Space connection has to land there too).
export const GET = withDepCutAuth(async (request: DepCutAuthenticatedRequest, context: RouteContext) => {
  ...
  if (!(await isBrandSpaceManager(request.depcut.userId, id))) {
