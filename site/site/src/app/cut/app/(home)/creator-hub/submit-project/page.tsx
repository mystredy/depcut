...
import {
  useCreateSubmission,
  useUploadSubmissionThumbnail,
  useUploadSubmissionVerification,
...
export default function SubmitProjectPage() {
  ...
  const uploadThumbnail = useUploadSubmissionThumbnail();
  ...
  const uploadVerification = useUploadSubmissionVerification();
  ...
  // Uploads are plain browser fetch/XHR calls tied to this page — a refresh
  // or tab close mid-upload silently kills them with no retry. Warn instead
  // of letting that happen invisibly.
  const uploadingInFlight =
    uploadThumbnail.isPending || uploadVideo.isPending || uploadVerification.isPending;
  useEffect(() => {
  ...
  // Scopes every upload made before a Submission row exists — uploads start
  // the instant a file is picked. The key an upload lands at is also its
  // permanent home once submitted (no copy step), so a fresh id is minted
  // after each successful submit — reusing one would let the next pick
  // overwrite media a past submission still points at.
  ...
  const couponFormatValid =
    couponCode.trim().length >= 8 && startsWithTwoLetters && isAlphanumeric;
  ...
  const canSubmit = Boolean(
    title.trim() &&
      ...
      !uploadingInFlight &&
      (!isProMode ||
        (mediaFile && verificationKey && packageTitle.trim() && description.trim() && tags.trim())) &&
  ...
  const pickVideo = (file: File) => {
    ...
    uploadVideo.mutate(
      { draftId, file, onProgress: setVideoUploadProgress },
      {
        onSuccess: ({ key }) => setVideoKey(key),
    ...
    toThumbnail(file)
      .then((blob) =>
        uploadThumbnail.mutate(
          { blob, draftId, onProgress: setThumbnailUploadProgress },
          {
            onSuccess: ({ key }) => setThumbnailKey(key),
  ...
  const pickVerification = (file: File) => {
    ...
    uploadVerification.mutate(
      { draftId, file, onProgress: setMediaUploadProgress },
      {
        onSuccess: ({ key }) => setVerificationKey(key),
        onError: () =>
          setVerificationUploadError("Couldn't upload the verification export — try again."),
  ...
  const generatePackage = async () => {
    ...
    try {
      ...
      if (descBlock?.trim()) setDescription(descBlock.trim());
      if (tagsBlock?.trim()) setTags(tagsBlock.trim());
      void queryClient.invalidateQueries({ queryKey: creditBalanceQueryKey });
    } catch (error) {
      if (error instanceof ApiError && error.code === "insufficient_credits") {
        setAiError("Not enough AI credits for that. Buy more from Billing.");
  ...
  const disconnect = (id: string) => {
    setWorkspaces((prev) =>
      prev.map((w) => (w.id === id ? { ...w, connected: false, workspaceName: "", connectedEmail: "" } : w))
    );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!canSubmit) return;
    setStatus(null);
    // The video, thumbnail, and (Pro) verification export already finished
    // uploading straight to R2 by now — they started the instant each file
    // was picked (see pickVideo/pickThumbnail/pickVerification). This call
    // just attaches the resulting keys to the new Submission row.
    createSubmission.mutate(
      {
        title: title.trim(),
        categoryId: category,
        subSource: submissionType === "Inspire" ? "InspiredExternal" : "TaskExternal",
        inspireUrl: (submissionType === "Inspire" ? inspirationLink : taskReference).trim(),
        voiceScript: vocalScript.trim(),
        spaceid: selectedWorkspaceId,
        videofile: videoFile?.name,
        thumbnailFile: thumbnailFile?.name,
        thumbnailKey: thumbnailKey ?? undefined,
        videoKey: videoKey ?? undefined,
        extension: isProMode ? "pro" : "standard",
        editCode: isProMode ? couponCode.trim() || undefined : undefined,
        watermarkEnabled,
        watermarkText: watermarkEnabled ? watermarkText.trim() : undefined,
        burnInCaptions,
        generatedMetadata: generatedTitles.length > 0,
        packageTitle: isProMode ? packageTitle.trim() : undefined,
        description: isProMode ? description.trim() : undefined,
        tags: isProMode ? tags.trim() : undefined,
        mediaFile: isProMode ? mediaFile?.name : undefined,
        verificationKey: isProMode ? (verificationKey ?? undefined) : undefined,
      },
      {
        onSuccess: () => {
          setStatus("Submitted! Your project is now pending review.");
          setTitle("");
          setCategory("");
          setPackageTitle("");
          setDescription("");
          ...
          setWatermarkText("");
          setBurnInCaptions(false);
          setMediaFile(null);
          setVerificationKey(null);
          // The media just uploaded is now permanently owned by that
          // submission — mint a fresh draft for whatever's picked next.
          setDraftId(crypto.randomUUID());
        },
        onError: () => {
          setStatus("Couldn't submit that just now. Try again in a moment.");
        },
      }
    );
  };
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <Select value={category} onValueChange={setCategory} disabled={categories.isLoading}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  submissionType === "Inspire"
                    ...
                    : setTaskReference(e.target.value)
                }
                placeholder={
                  submissionType === "Inspire"
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="submit-script"
                value={vocalScript}
                onChange={(e) => setVocalScript(e.target.value)}
            ...
            <div className="space-y-4 rounded-2xl border border-primary/20 bg-primary/5 p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    disabled={!couponFormatValid || couponChecking}
                ...
                <>
                  ...
                  <div className="flex items-center justify-between gap-4 rounded-xl border bg-background p-3">
                    ...
                    <Switch
                      ...
                      onCheckedChange={setBurnInCaptions}
              ...
              <div className="space-y-2 rounded-xl border bg-background p-3">
                ...
                <Button
                  ...
                  size="sm"
                  className="w-full"
                  disabled={generating === "package" || !vocalScript.trim()}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <AiButton
                    ...
                    label="Generate"
                    disabled={!vocalScript.trim() || !packageTitle.trim()}
                    title={
                      !vocalScript.trim()
          ...
          <Button type="submit" disabled={!canSubmit || createSubmission.isPending} className="w-full">
        ...
        <div className="space-y-4 rounded-3xl border bg-card p-6 lg:col-span-4">
          ...
          {connectedWorkspaces.length === 0 ? (
            <div className="space-y-2 rounded-xl border border-dashed p-4 text-center">
              ...
              <Button type="button" size="sm" variant="outline" className="w-full" onClick={() => setConnectOpen(true)}>
                Link Workspace
              </Button>
            </div>
          ) : (
              ...
              {connectedWorkspaces.map((w) => (
                <div key={w.id} className="flex items-center justify-between gap-2.5 rounded-xl border bg-muted/30 p-3">
                  ...
                  <button
                    ...
                    className="shrink-0 text-[10px] font-semibold text-destructive hover:underline"
                  >
                    Unlink
                  </button>
                </div>
