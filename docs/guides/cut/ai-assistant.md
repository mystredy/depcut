...
| Tool catalog (88 tools) | ~75K chars (~19K tokens) | Claude/Codex: the MCP tool listing. Gemini: function declarations on each request. |
...
`extend_video` renders a continuation from a clip's own last frame — its crop, pan, and color grade applied, no other track or overlay baked in — through the same async job path every other video tool uses. Only a model that declares extend support runs it; today that is Veo's tiers, one direction (a clip's end) and three durations (4, 6, or 8 seconds), and nothing outside that set gets silently rounded onto one of them. A finished continuation is never placed automatically: it previews like any render, and only `add_to_timeline` (or the timeline's own AI Extend action) inserts it, as a ripple-safe clip immediately after its source — a transition the source held into its old neighbor moves to the new boundary — in one undo step.

Object-level generative editing — recoloring, removing, or replacing something inside a clip, tracked across frames — has no model wired up yet. A capability interface sketches the seam for one without pretending it works today.
