...
import { useBrandSpaceByUsername, useBrandSpacePosts } from "@/queries/brandSpace";

// A Brand Space's public profile — same shape as My Space (avatar, bio,
// posts grid), plus a Settings entry point for whoever manages it. No
// avatar/background image upload yet (BrandSpace.avatarImageKey stays
// null until that's built) — UserAvatar's initial-letter fallback covers
// it in the meantime.
export default function BrandSpacePage({ params }: { params: Promise<{ username: string }> }) {
  const { username } = use(params);
  const base = useCutBase();
  const { data, isLoading } = useBrandSpaceByUsername(username);
  const posts = useBrandSpacePosts(data?.space.id ?? "");
  ...
  return (
    <div className="mx-auto max-w-2xl pb-24">
      ...
      {posting && <PostToSpaceDialog projectId={null} brandSpaceId={space.id} onClose={() => setPosting(false)} />}
    </div>
  );
}
