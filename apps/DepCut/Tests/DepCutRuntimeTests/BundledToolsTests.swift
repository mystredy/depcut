...
/// an explicit `DEPCUT_TOOLS_DIR` (how a test process points at the repo's vendor copy) wins, otherwise
/// the app's own `Resources/depcut-tools`. These lock that order and the "are the tools really here" check.
@Suite
struct BundledToolsTests {
    @Test
    func overrideWinsAndMustExist() {
        let real = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("depcut-tools-test-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: real, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: real) }

        setenv("DEPCUT_TOOLS_DIR", real.path, 1)
    ...
    func theSwiftListMatchesWhatTheBuildStages() throws {
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()  // DepCutRuntimeTests
            .deletingLastPathComponent()  // Tests
            .deletingLastPathComponent()  // DepCut
