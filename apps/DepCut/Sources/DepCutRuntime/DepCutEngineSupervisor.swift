...
///
/// Cut is free and standalone, so the engine runs regardless of DepCut sign-in state.
public final class DepCutEngineSupervisor: @unchecked Sendable {
    /// The loopback port this build's engine binds, probes, and evicts stale engines on. The dev
    /// app sets DepCutEnginePort in its Info.plist (scripts/run-depcut-dev.sh) so its engine
    ...
    private static let port: Int = {
        let raw = Bundle.main.object(forInfoDictionaryKey: "DepCutEnginePort")
    ...
    private let queue = DispatchQueue(label: "depcut.cut-engine-supervisor")
    ...
    /// `nil` (e.g. a dev build made without the site checkout) just means DepCut runs without Cut.
    private static func engineBinary() -> URL? {
        let fileManager = FileManager.default
        if let raw = getenv("DEPCUT_CUT_ENGINE_BIN") {
        ...
        guard let baked = Bundle.main.resourceURL?
            ...
            .appendingPathComponent("depcut-cut-engine")
        else { return nil }
    ...
    private func servedEngine() -> (version: String, user: String?)? {
        ...
        URLSession.shared.dataTask(with: request) { data, _, _ in
            ...
            guard let data,
                  ...
                  payload["engine"] as? String == "depcut-cut"
            else { return }
    ...
    private func spawnIfNeeded() {
        ...
        environment["DEPCUT_CUT_ENGINE"] = "1"
        environment["DEPCUT_CUT_VERSION"] = Self.appVersion
        environment["DEPCUT_CUT_PORT"] = String(Self.port)
        environment["DEPCUT_CUT_PARENT_PID"] = String(ProcessInfo.processInfo.processIdentifier)
        if let tools = BundledTools.resolvedDirectory() {
            environment["DEPCUT_CUT_TOOLS_DIR"] = tools.path
        }
    ...
    /// Engine stdout/stderr land in ~/Library/Logs/DepCut/cut-engine.log; truncated when it grows
    ...
    private static func openLog() -> FileHandle? {
        ...
        let dir = fileManager.urls(for: .libraryDirectory, in: .userDomainMask)[0]
            ...
            .appendingPathComponent("DepCut", isDirectory: true)
