# Fast DepCut: tool-first Command Layer + always-on Gemini Live
...
DepCut is text-first, but slow to *act*. Historically it could only act two ways: generate
...
`DepCutCommandBackends` enforces single-line input, a length cap, and a hard ~12s timeout
...
**Where it lives / how it's wired.** `DepCutHarness` only depends on `DepCutContracts` and can't
import AppKit, so it follows the codebase's existing injected-closure pattern (like
`appleScriptExecutor`):

- `DepCutHarness/DepCutCommandLayer.swift` — tool descriptors + the shared `Command` enum
  (single source of truth for the command names).
- `DepCutRuntime/DepCutCommandBackends.swift` — the native side effects.
...
- `DepCut/GeminiLiveVoiceController.swift` — owns the session, runs the tool-call loop against a
...
- **Vertex AI** (no key — production): the backend authenticates with Google Cloud OAuth
  (service-account) credentials. `DepCutBackendInferenceClient.mintLiveConnection()` →
...
against a live backend + Vertex. It is gated behind `DEPCUT_LIVE_SMOKE=1` and no-ops otherwise:

```
env DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
  DEPCUT_LIVE_SMOKE=1 DEPCUT_WEB_BASE_URL=http://localhost:3000 DEPCUT_DEV_AUTH_BYPASS=1 \
