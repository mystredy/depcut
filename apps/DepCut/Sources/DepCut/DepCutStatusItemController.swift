import AppKit
import DepCutRuntime

/// The menu bar entry point: a status item with the DepCut glyph whose menu carries "Go to App",
...
final class DepCutStatusItemController: NSObject, NSMenuDelegate {
    ...
    var updateState: DepCutUpdateState? {
        didSet {
    ...
    func populateMenuItems(in menu: NSMenu) {
        menu.addItem(makeItem(title: "Go to App", action: #selector(goToAppAction), image: Self.depcutMenuIcon()))
        ...
        menu.addItem(.separator())
        menu.addItem(makeItem(title: "Quit DepCut", action: #selector(quitAction), keyEquivalent: "q"))
    }
    ...
    /// The depcut glyph sized for a menu row, as a template so the menu recolors it for light/dark.
    private static func depcutMenuIcon() -> NSImage {
        let glyph = loadGlyph()
    ...
    /// The depcut glyph as a template image: pure black plus alpha, so the system recolors it
    ...
    /// The dev build packages its own bundle identifier (com.depcutuse.DepCut.dev), so a `.dev`
    /// suffix marks this as the dev app.
    private static let isDevBuild = Bundle.main.bundleIdentifier?.hasSuffix(".dev") ?? false

    /// The 20pt depcut glyph, both resolutions, with no template flag set yet.
    private static func loadGlyph() -> NSImage {
        let image = NSImage(size: NSSize(width: 20, height: 20))
        for resource in ["menu-bar-icon", "menu-bar-icon@2x"] {
            guard let url = DepCutResourceBundle.app?.url(forResource: resource, withExtension: "png"),
    ...
    /// The depcut glyph with a red silhouette outline. Not a template image — a template discards
