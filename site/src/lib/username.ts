...
// digits, underscores, and dots, 3-20 characters, starting with a letter.
// superRefine reports the one rule that actually failed instead of dumping
// the whole spec at every rejection.
export const usernameSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (badChar) {
      const label = badChar === " " ? "Spaces" : `"${badChar}"`;
