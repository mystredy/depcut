...
export type VisualGenerationRecordBase = {
  tool: "text-to-image" | "text-to-video";
  prompt: string;
  aspect: string;
  tier: string;
};
...
/** Best-effort: save a just-settled Text to Image/Video take (succeeded or
 * failed) to the database. Called once per take, right after it settles,
 * alongside — not instead of — the browser-local Recent History entry. A
...
export async function persistVisualGeneration(record: VisualGenerationRecord): Promise<void> {
  try {
    ...
    await hostedPost("/api/visual-generations", {
      ...rest,
      dataBase64,
      mimeType: blob.type || (record.tool === "text-to-image" ? "image/png" : "video/mp4"),
    });
  } catch {
