...
3. Points below ~0.5 confidence are unreliable (an instrumental break, a spoken intro one source doesn't have, a key change) — don't cut on those blindly. listen_audio around a candidate cut point on the sources involved to confirm what's actually there and that it isn't mid-word; watch_video the shot to confirm it doesn't land mid-motion.
4. Walk the reference's timeline and pick cut points where enough sources have a confident match, each segment at least as long as asked (default ~5s), rotating through every source in the mix (not just two) end to end through the requested section — a source with no reliable match for a stretch just sits out that segment.
5. Build it in order: for each segment, add_clip the asset (appended at the end) then trim_clip its in/out to that source's matched window — a clip's timeline length is (out-in)/speed, so it lands right after the previous segment with no gap to close.
