...
- \`model\` picks the engine — default "gemini" (DepCut's hosted voices, fast, supports direction/tags/language) or an ElevenLabs model ("eleven_v3", "eleven_multilingual_v2" — the same two the ai-suite text-to-speech page offers). Gemini voices are a fixed prebuilt set — list_voices returns them (id + one-word character like Warm, Upbeat, Gravelly); omit voice for a good default. ElevenLabs voices are the user's own account library — list_voices provider:"elevenlabs" fetches them, and one is REQUIRED (no built-in default exists); an account with none set up needs the user to add one first (the ai-suite page, or their ElevenLabs account). \`direction\`, inline tags, and \`language\` pronunciation pinning are gemini-only — ElevenLabs speaks the script exactly as written. Reach for ElevenLabs when the user names it or a voice only their account has; default to gemini otherwise.
