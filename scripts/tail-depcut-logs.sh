...
usage() {
  cat <<'EOF'
Tail DepCut logs from macOS Unified Logging.

Usage:
  scripts/tail-depcut-logs.sh [options]

Options:
  --process NAME       Process name to filter. Default: DepCut
...
Examples:
  scripts/tail-depcut-logs.sh
  scripts/tail-depcut-logs.sh --errors
  scripts/tail-depcut-logs.sh --last 10m --errors
  scripts/tail-depcut-logs.sh --pid 88494 --debug
EOF
}
...
# tag, which are identical on every DepCut line and just add noise. When writing to a
