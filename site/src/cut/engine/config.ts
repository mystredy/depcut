...
/**
 * The port the engine binds, from DEPCUT_CUT_PORT when set. A bad value throws
...
export function enginePort(): number {
  const raw = process.env.DEPCUT_CUT_PORT;
  if (raw === undefined || raw === "") return DEFAULT_ENGINE_PORT;
  const n = Number(raw);
  if (!Number.isInteger(n) || n < 1 || n > 65535) {
    throw new Error(`DEPCUT_CUT_PORT must be an integer 1–65535, got "${raw}".`);
  }
  return n;
}

/** The engine's own loopback base URL (honors DEPCUT_CUT_PORT). */
