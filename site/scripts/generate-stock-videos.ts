...
async function writeManifest(done: Set<string>, tags: Map<string, string[]>) {
  const entries = CATALOG.filter((c) => done.has(c.id)).map((c) => ({
    ...
    // local public/cut-stock-video these files land in when generated —
    // upload the new output there after a regeneration.
    file: `/api/stock-media/cut-stock-video/${c.id}.mp4`,
    thumb: `/api/stock-media/cut-stock-video/${c.id}.thumb.webp`,
