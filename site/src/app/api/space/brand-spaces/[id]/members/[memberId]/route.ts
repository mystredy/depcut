...
// delete the space instead of trying to remove the last owner).
export const DELETE = withDepCutAuth(async (request: DepCutAuthenticatedRequest, context: RouteContext) => {
  ...
  const target = await prisma.brandSpaceMember.findUnique({
    include: { user: { select: { name: true } } },
    where: { id: memberId },
  });
  if (!target || target.brandSpaceId !== id) return notFoundResponse();
  ...
  await prisma.brandSpaceMember.delete({ where: { id: memberId } });
