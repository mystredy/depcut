...
// The transport adapter: pi Context in, DepCut's hosted Responses route out.
...
export interface DepCutToolDetails {
  response: unknown;
  mediaParts?: WirePart[];
}
...
export function serializeMessages(messages: Message[]): WirePart[] {
  ...
  for (const m of messages) {
    if (m.role === "toolResult") {
      const details = (m.details ?? {}) as DepCutToolDetails;
      const response =
...
export interface DepCutStreamDeps {
  post: PostFn;
...
export function makeDepCutStream(deps: DepCutStreamDeps) {
