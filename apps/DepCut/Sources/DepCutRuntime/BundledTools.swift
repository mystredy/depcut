...
///
/// They ship inside the app: `DepCut.app/Contents/Resources/depcut-tools`, staged at packaging time by
/// scripts/package-depcut-app.sh. Being part of the app means they are there the moment it is — no
/// first-run download to race, nothing to retry on a bad network, and no dependence on what the user
/// happens to have installed. A dev build symlinks that same path at the repo's `vendor/depcut-tools`,
/// so a source checkout and a shipped app resolve identically.
public enum BundledTools {
    /// Bare executable names of the tools shipped in the bundle. DepCut resolves these from
    ...
    public static var appDirectory: URL? {
        Bundle.main.resourceURL?.appendingPathComponent("depcut-tools", isDirectory: true)
    }

    /// The tools directory this process should use, or nil when it has none.
    ///
    /// `DEPCUT_TOOLS_DIR` comes first, and is what lets a test or eval process — where `Bundle.main` is
    /// the test runner, so the app path never resolves — point at the repo's `vendor/depcut-tools` and
    /// run the same bundled binaries as production. It mirrors the
    /// build-time `DEPCUT_TOOLS_DIR` convention in `ensure-bundled-tools.sh`.
    public static func resolvedDirectory() -> URL? {
        ...
        if let raw = getenv("DEPCUT_TOOLS_DIR") {
    ...
    private static func resolveLoginShellPath() -> String {
        ...
        let start = "__DEPCUT_PATH_START__"
        let end = "__DEPCUT_PATH_END__"
        let process = Process()
