...
extension NSScreen {
    ...
    var depcutDisplayID: CGDirectDisplayID? {
...
final class RegionSelectionOverlayController {
    ...
    func begin() {
        ...
        for screen in NSScreen.screens {
            guard let displayID = screen.depcutDisplayID else { continue }
            ...
            panel.level = DepCutOverlayWindowLevel.regionSelection
...
final class RecordingRegionDimOverlayController {
    ...
    func show(region: CGRect, displayID: CGDirectDisplayID) {
        close()
        guard let screen = NSScreen.screens.first(where: { $0.depcutDisplayID == displayID }) else { return }
