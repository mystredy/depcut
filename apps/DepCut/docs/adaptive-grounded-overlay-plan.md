...
DepCut can drive scriptable apps via AppleScript (e.g. Music, now playing reliably through the
...
- Users also ask **"show me where X is" / "how do I X"** — a guidance intent where DepCut should
  *point*, not act.
...
| Capability | Where |
| --- | --- |
| Cursor overlay window + animation | `PointerCoachCursorOverlayController` (DepCut), `PointerCoachCursorOverlayView`/`…ViewModel` (DepCutUI) |
| Visualize tool (grounded steps → overlay request) | `agent.path.visualize` in `GenericHarnessBuiltInToolExecutors`; `AgentPathStep`/`AgentVisualizationPlan` (DepCutContracts) |
...
### B. AX-grounded path builder (the shared foundation) — new in DepCutRuntime
