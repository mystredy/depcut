...
import { presignPut, spacePostVideoKey } from "@/cut/server/cloud/r2";
...
// Mints a presigned PUT for a post's video, gated on the account's 10GB
// quota — checked here, before a byte moves, rather than after upload
// (submissions' presign has no such gate; a post's does, since nothing
// else caps how much a Space can hold).
export const POST = withDepCutAuth(async (request, context: RouteContext) => {
  ...
  const key = spacePostVideoKey(userId, id, parsed.data.fileName);
