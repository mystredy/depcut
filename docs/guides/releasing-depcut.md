...
DepCut production releases are distributed through GitHub Releases, built by
the `Release DepCut` GitHub Actions workflow from the latest default-branch
...
`Nightly DepCut Build` workflow publishes a smoke-test prerelease on a
...
`/releases/download/vMAJOR.MINOR.PATCH/DepCut.dmg`, never a moving `latest` or
`-latest` URL. Alias tags exist only for maintainer convenience.

## Workflows

| | `Release DepCut` | `Nightly DepCut Build` |
|---|---|---|
...
| Release | numeric GitHub Release, marked GitHub's latest | `DepCut Nightly Build` prerelease |
| Assets | `DepCut.dmg` + `DepCut.dmg.sha256` | `DepCut.dmg` + `DepCut.dmg.sha256` |
...
`Release DepCut` to publish a user-facing release.

## What Triggers a Release

The commit says so. A subject ending in ` [rebuild]` means the change ships
inside the Mac app — the Swift app, the DepCut engine sources compiled into the
...
`dist/DepCut.dmg`, Developer ID-signs and notarizes the app and disk image and
...
The DepCut engine runs CLI tools — `ffmpeg` and `ffprobe` behind every export, probe,
and frame extract, and `yt-dlp` behind URL import — by bare name off PATH. Those tools ship **inside** the app, at
`DepCut.app/Contents/Resources/depcut-tools`. Being part of the app is what makes
...
tools in `vendor/depcut-tools` — downloading the prebuilt bundle named in
`bundled-tools.json` when one is published, otherwise building from source — and
`package-depcut-app.sh` copies that directory into the app and re-signs it. Point
`DEPCUT_TOOLS_DIR` at a prebuilt directory to bake that instead.
...
When the manifest moves, the workflow asks `Release DepCut` for a patch release
...
1. **Developer ID Application certificate.** Create it in Xcode: Settings →
   ...
   - `DEPCUT_DEVELOPER_ID_CERT_P12` = `base64 -i cert.p12`
   - `DEPCUT_DEVELOPER_ID_CERT_PASSWORD` = the export password
   - `DEPCUT_TOOLS_SIGN_IDENTITY` = the identity, e.g. `Developer ID Application: Name (TEAMID)`
2. **App Store Connect API key** (notarization), from App Store Connect → Users
   ...
   - `DEPCUT_NOTARY_KEY_P8` = `base64 -i AuthKey_XXXX.p8`
   - `DEPCUT_NOTARY_KEY_ID` = the key's Key ID
   - `DEPCUT_NOTARY_ISSUER_ID` = the Issuer ID
...
```bash
apps/DepCut/.build/artifacts/sparkle/Sparkle/bin/generate_keys --account depcut
```

Copy the printed `SUPublicEDKey` value into the GitHub repository secret
`DEPCUT_SPARKLE_PUBLIC_ED_KEY`. Then export the matching private key:

```bash
apps/DepCut/.build/artifacts/sparkle/Sparkle/bin/generate_keys \
  --account depcut \
  -x /tmp/depcut-sparkle-private-key.txt
```

Copy the file contents into the GitHub repository secret
`DEPCUT_SPARKLE_PRIVATE_ED_KEY`, then delete the exported file:

```bash
rm -f /tmp/depcut-sparkle-private-key.txt
```
...
Sparkle signs and validates update archives; do not add a DepCut-specific
...
- The GitHub Release for `vMAJOR.MINOR.PATCH` contains `DepCut.dmg` and
  `DepCut.dmg.sha256`.
- The website source has `DEPCUT_LATEST_VERSION` set to the numeric version.
- `site/public/appcast.xml` contains one item for that version and an
  enclosure URL under `/releases/download/vMAJOR.MINOR.PATCH/DepCut.dmg`.
...
- `https://depcut.com/appcast.xml` updates after the site deploy completes.

If Sparkle cannot validate an update, first check that the app was packaged
with the `DEPCUT_SPARKLE_PUBLIC_ED_KEY` that matches the private key stored in
GitHub Actions.
