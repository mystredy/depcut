#!/bin/bash
# Compile the DepCut engine into single-file Mac binaries.
#
...
# dist/cut-engine/depcut-cut-engine-<arch>. The DepCut app bundles the binary and
# spawns it with DEPCUT_CUT_ENGINE=1 (see plans/cut-engine.md).
set -euo pipefail
cd "$(dirname "$0")/.."
...
for arch in "${arches[@]}"; do
  out="dist/cut-engine/depcut-cut-engine-$arch"
