// Public DepCut Vision API contract for POST /api/vision.
...
export const CODE_SAMPLES: CodeLanguage[] = [
  ...
  method: "POST",
  headers: {
    Authorization: \`Bearer \${process.env.DEPCUT_API_KEY}\`,
...
res = requests.post(
    "${url}",
    headers={"Authorization": f"Bearer {os.environ['DEPCUT_API_KEY']}"},
    json={
        # base64 png/jpeg/webp, no "data:" prefix
...
curl "${url}" \\
  -H "Authorization: Bearer $DEPCUT_API_KEY" \\
