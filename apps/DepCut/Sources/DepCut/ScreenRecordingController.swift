...
final class ScreenRecordingController {
    ...
    /// The "Record Screen" item's title and matching record/stop glyph for the DepCut menu, or `nil`
    ...
    /// Routes a click on the DepCut menu's "Record Screen" item: open the control bar, or stop a
    ...
    private func resolveTarget() -> ScreenCaptureTarget? {
        switch model.mode {
        case .fullScreen:
            guard let displayID = (armScreen ?? NSScreen.main)?.depcutDisplayID else {
